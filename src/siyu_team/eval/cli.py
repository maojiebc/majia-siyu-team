...
def cmd_score(args) -> int:
    if not os.path.exists(args.file):
        print("找不到文件:", args.file); return 2
    ...
    if st["hard_fail"]:
        print("❌ COMPLIANCE_RED：命中合规红线，方案不得交付。"); return 1
    ...
    if approx < args.threshold:
        print("低于阈值 %d，打回。" % args.threshold); return 1
    return 0


def cmd_validate(args) -> int:
    """校验 SKILL.md 的 frontmatter、目录名和 8KB 体积上限。"""
    root = args.path; problems = 0
    for dirpath, _, files in os.walk(root):
        for fn in files:
            ...
            match = re.match(r"^---\s*\n(.*?)\n---\s*\n", text, re.DOTALL)
            if not match:
                print("⚠️ 缺 frontmatter:", p); problems += 1
                continue
            name_match = re.search(r"^name:\s*([^\n]+)$", match.group(1), re.MULTILINE)
            if not name_match:
                print("⚠️ 缺 frontmatter name:", p); problems += 1
            else:
...
def main() -> None:
    ...
    s = sub.add_parser("score"); s.add_argument("file"); s.add_argument("--threshold", type=int, default=80)
    s.set_defaults(func=cmd_score)
    v = sub.add_parser("validate"); v.add_argument("path", nargs="?", default="plugins/")
