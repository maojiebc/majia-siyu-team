...
- `retail` 额外加载种子层 `knowledge/02-industry/retail/L1-零售私域种子.md` 与社区语料；路由仍是 `generic_only`。零售行业册：敬请期待，当前为种子层，由社区印证逐步替换。
- 社区语料在 `knowledge/05-community/`，不进入本目录的 35 条严格正式集。装配时严格原子在前，社区原子按证据等级标注后补位，总数仍不超过 12。
