...
## [1.5.0] - 2026-09-14
### 新增
- 门店会员场景接入正常入口：收银、领礼进群、开业演练、核销排错、员工激励和店长读数；补充日常内容与储值诉求。
- 严格公开知识集 0.5.0：更新 3 条原有方法、新增 6 条，共 41 条；每条保留条件、指标、例外、来源与编辑审阅信息。
- 同行共建入口与评审状态随本版分发；只读取批准内容，待审、撤销及私有资料不进入安装包。
### 归整
- 同步插件、Python、SkillHub 和 WorkBuddy 版本，更新使用入口、能力图与发布说明；澄清正常路由仍在运行并接受回归检查。
- 计算规则与会员数据顾问相互指引，原文、真实业务数字、企业条款及未核验附件不随包公开。
### 验证边界
- 六个场景已验证源码入口、独立 wheel 安装和 SkillHub 查询；编辑与工程验证不等于经营增量证明。
- H1 知识价值、H2 贡献动力、H3 审核吞吐仍为 Not Evaluated；不能把提交、审批或安装成功当作这些指标通过。
...
- 公开知识改为随 wheel 分发的规范副本，manifest、schema、hash 与 approved 原子严格校验；缺失或损坏时失败关闭，不回退 draft；Runtime 按任务相关性、业态与绑定能力装配。
- 合规词表改为上下文判定，避免把“第一步”“第一周”“不是最优方案”等正常表达误判为硬红线；`make eval` 明确只做静态门，Judge 评分需显式输入宿主评审结果。
- 用户输入、外部证据和四位专家输出均进入不可信内容边界；主持人只把专家输出当数据；上下文体积和专家最低结构字段受到约束。
- Trace 默认只落最小元数据，原文记录必须显式选择 `full`；状态文件加入并发、旧格式只读和路径安全保护；存档默认只保存结构化结论并要求写盘前隐私选择。
- 增加干净 wheel、SkillHub 安装态、运行链路与发布契约回归。线上发布仍由人工执行，并要求同一 commit 与 SemVer。

### 评估边界
- H1 知识价值、H2 贡献动力、H3 审核吞吐均为 **Not Evaluated**；本版只证明工程工具与评估协议就绪，不声称知识飞轮或同行共建已经成立。
